...
- Use Azure Event Hubs Standard with Kafka endpoints for vitals and prescription events.
- Use Event Hubs Capture to ADLS Gen2 as the durable streaming replay archive, exposed to the Bronze
  Lakehouse through a OneLake shortcut.
- Use Fabric Eventstream for streaming connection and routing, and Eventhouse with a KQL database for
  hot operational analytics.
...
| Streaming ingress | Kafka-compatible endpoint | Azure Event Hubs | Isolated vitals and prescription event hubs |
| Streaming archive | Replay store | Event Hubs Capture to ADLS Gen2 | Durable replay and recovery archive |
| Streaming routing | Stream connection and routing | Fabric Eventstream | Route and lightly shape event streams |
...
| Bronze | Immutable source-aligned layer | Fabric Bronze Lakehouse | Raw batch files, captured events through shortcut, metadata |
...
| Monitoring | Azure streaming ingress | Azure Monitor alert rules plus Action Groups | Event Hubs throughput, availability, throttling, and on-call routing |
...
1. Bedside monitors and the pharmacy system publish events to separate Azure Event Hubs through the
   Kafka-compatible endpoint. Proposed ADT/bed-state and staffing-actual feeds join this path only
   after source contracts are approved.
2. Event Hubs Capture archives the source stream to ADLS Gen2 for replay and recovery.
3. A OneLake shortcut exposes captured data to the Bronze Lakehouse for historical processing.
4. Fabric Eventstream routes live events to Eventhouse.
5. KQL functions and update policies validate, deduplicate, and separate valid and quarantined
   events using stable event identifiers and event time.
...
- Azure Monitor alert rules route Event Hubs health alerts through Azure Monitor Action Groups; they
  do not route through Fabric Activator by default.
- Duplicate files or events are detected using source identifiers, checksums, and stable event IDs.
- Streaming replay reads from Event Hubs Capture rather than relying on the hot-store retention
  window. A controlled replay job republishes captured events with their stable event IDs to rebuild
  the hot path without duplicating accepted events.
...
- Each environment is an isolated stack including Fabric workspaces, Event Hubs, replay storage,
...
- Select Fabric and Event Hubs capacity tiers after representative performance testing.

...
> **Superseded (2026-08-26): no Azure Event Hubs.** The original design put Azure Event Hubs in
> front of Eventstream as the Kafka-compatible endpoint, with Event Hubs Capture → ADLS Gen2 →
> a OneLake shortcut as the replay archive. `GENERATOR-README.md` flagged this explicitly: it adds
> an Azure resource, its own cost/capacity tier, and a hop that buys nothing Eventstream's own
> native custom endpoint doesn't already provide for this workload. The three sections below
> describe what was replaced; "Fabric Eventstream" describes what's actually built.

### Fabric Eventstream (Kafka-compatible endpoint, routing, and archive fan-out)
- **What it is:** A no/low-code Fabric item that can itself expose a Kafka-compatible custom
  endpoint as a source, then route/lightly transform events before they land somewhere else.
- **Function here:** The landing point for two continuous producers — bedside vital-sign
  monitors and pharmacy prescription-issuance events — published over the Kafka protocol as two
  logical topics (`patient-vitals`, `prescription-events`), so existing Kafka producer libraries
  work unchanged. Eventstream filters by `event_type` into two derived streams, each fanned out
  to **two** destinations: the Eventhouse (hot path) and a Bronze Lakehouse Delta table (durable
  replay archive — written straight into OneLake, so no separate ADLS resource or shortcut hop
  is needed to make it available to Bronze-style processing).
...
| **Ingestion service** | Dataflow Gen2 (documents) / Fabric Data Factory Copy activity (extracts) | Fabric Eventstream custom endpoint (Kafka-compatible) |
| **Durable raw copy** | Bronze Lakehouse itself is the immutable copy | Eventstream's second destination writes raw events straight to a Bronze Lakehouse Delta table |
...
| **Recovery/replay mechanism** | Rerun pipeline from checkpoint; Bronze is already replayable | Controlled replay job reads from the Bronze Lakehouse archive table and republishes with original event IDs to rebuild the hot path without duplicating already-accepted events |
...
| **Azure Monitor + Action Groups** | Not typically needed — Fabric-native monitoring covers batch | Watches Eventstream throughput/throttling/availability where Fabric-native monitoring doesn't cover it |
